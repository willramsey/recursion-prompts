...
var arraySum = function(array) {
  ...
  return result + arraySum(array.slice(1));
};
...
var palindrome = function(str) {
  var string = str.trim().toLowerCase();
...
var modulo = function(x, y) {
  if (x >= 0 && y > 0) { return x >= y ? modulo(x - y, y) : x; }
  if (x > 0 && y < 0) { return x >= -y ? modulo(x + y, y) : x; }
  if (x < 0 && y > 0) { return -x >= y ? modulo(x + y, y) : x; }
  if (x < 0 && y < 0) { return x <= y ? modulo(x - y, y) : x; }
};
...
var multiply = function(x, y) {
  if (x === 0 || y === 0) { return 0; }
  if (y < 0 && x > 0) { return multiply(x, y + 1) - x; }
  if (y > 0 && x < 0) { return multiply(x + 1, y) - y; }
...
var fibonacci = function(n) {
  if (n <= 0) { return null; }
  if (n === 1) { return [0, 1]; }
  var returnValue = fibonacci(n - 1);
  var len = returnValue.length;
  var fibNum = returnValue[len - 1] + returnValue[len - 2];
  return returnValue.concat([fibNum]);
...
var capitalizeWords = function(array) {
  if (array.length === 0) { return []; }
  var upper = array[0].toUpperCase();
  return [upper].concat(capitalizeWords(array.slice(1)));
};
